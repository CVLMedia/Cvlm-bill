...
router.get('/dashboard/api/system-info', adminAuth, async (req, res) => {
  try {
    ...
    // Get CPU usage percentage - using top command for more accurate reading
    let cpuUsage = 0;
    try {
      // Try using top command first (most accurate)
      const { stdout: topOutput } = await execAsync("top -bn1 | grep 'Cpu(s)'");
      const cpuMatch = topOutput.match(/(\d+\.?\d*)%us,\s*(\d+\.?\d*)%sy,\s*(\d+\.?\d*)%ni,\s*(\d+\.?\d*)%id/);
      
      if (cpuMatch) {
        const user = parseFloat(cpuMatch[1]) || 0;
        const system = parseFloat(cpuMatch[2]) || 0;
        const nice = parseFloat(cpuMatch[3]) || 0;
        const idle = parseFloat(cpuMatch[4]) || 0;
      ...
      // Fallback: use /proc/stat method with longer delay
      try {
        const { stdout: stat1 } = await execAsync("cat /proc/stat | head -1");
        await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1 second for better accuracy
        ...
        // Final fallback: use load average as percentage of CPU cores
