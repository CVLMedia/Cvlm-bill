...
:local dnsApp [/ip dns static find where name="cvlmedia.my.id" and address="192.168.8.89"]
if ($dnsApp != "") {
    :put "✅ DNS static untuk cvlmedia.my.id: ADA"
    :put "   Address: [/ip dns static get $dnsApp address]"
    :put "   TTL: [/ip dns static get $dnsApp ttl]"
} else {
    :put "❌ DNS static untuk cvlmedia.my.id: TIDAK ADA"
}
