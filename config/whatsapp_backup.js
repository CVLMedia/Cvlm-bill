...
async function connectToWhatsApp() {
    try {
        ...
        sock = makeWASocket({
            auth: state,
            logger,
            browser: ['CV Lintas Multimedia', 'Chrome', '1.0.0'],
...
▸ *setheader [teks_header_baru]* — Ganti header pesan bot
   Contoh: setheader CV Lintas Multimedia
...
async function sendWelcomeMessage(remoteJid, isAdmin = false) {
    try {
        ...
        let welcomeMessage = `👋 *Selamat Datang di Bot WhatsApp ${getSetting('company_header', 'CV Lintas Multimedia') || 'ISP Monitor'}*\n\n`;
        ...
        // Tambahkan footer
        welcomeMessage += `🏢 *${getSetting('company_header', 'CV Lintas Multimedia') || 'ISP Monitor'}*\n`;
...
async function handleAdminMenu(remoteJid) {
    try {
        ...
        // Tambahkan footer
        adminMessage += `🏢 *${getSetting('company_header', 'CV Lintas Multimedia') || 'ISP Monitor'}*\n`;
